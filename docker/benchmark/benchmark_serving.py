...
def main(args: argparse.Namespace):
    ...
    if args.dataset_name == "sonnet":
        ...
        #     dataset_content = dataset.data

        #     # 打印数据集的前几个条目
        #     for i, entry in enumerate(dataset_content[:5]):  # 打印前 5 个条目
        #         print(f"Entry {i+1}: {entry}")

        #     # 打印数据集的总大小
        #     print(f"Total number of entries: {len(dataset_content)}")
        # else:
        #     print("ShareGPTDataset does not have a 'data' attribute.")
        # exit()
        
        # max_prompt_len = max(req.prompt_len for req in input_requests)
        # print(f"The maximum prompt_len is: {max_prompt_len}")
        ...
        # # 假设 ShareGPTDataset 有一个名为 data 的属性，存储数据集内容
        # if hasattr(temp, 'data'):
        #     dataset_content = temp.data
